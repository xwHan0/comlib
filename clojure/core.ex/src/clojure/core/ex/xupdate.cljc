...
;; Add by xwHan
(defn- expand-path-with-content-fn [state base keep?]
  (let [value (get-in state (:path base))
          value-pairs (cond (sequential? value) (into {} (map #(hash-map %1 %2) (range) value))
                            (associative? value) value)]
    (if-let [ks (->> value-pairs (filter #(keep? (key %) (val %))) keys)]
      (for [k ks] (append-vec base :path k)))))
;; End of Add
...
(defn expand-path
  ...
  (letfn [(expand-path-once [base crumb]
            (cond
             ...
             ;; Modify by xwHan
              (fn? crumb) (expand-path-with-content-fn state base crumb)
...
;Append by xwHan
(defn xupdate [coll xpath & args]
 "Read, delete and update collection data by instar/transform and get-in function.
  This is a extend from instar/transform.
  
  ## path syntax
  path ::= [path-form]
    path-form ::= keyword-name      ;For map keyword index
                  | position-number   ;For sequence keyword index
                  | regex         ;Pattern for map keyword or sequence position number
                  | func          ;Pred function: (idx val)=>pred?
                  | *             ;Match all"
  (if args
    (let [
      f (first args)
      pred (if (fn? f)
             #(apply (first args) % (next args))
             f)
      coll (if (sequential? coll) (vec coll) coll)
      ]
      (transform coll xpath pred))
    (get-in-paths coll xpath)))