...
from comlib import iterator, Index, ChildSub, ChildAttr, xiter
...
class TestIterator:
    ...
    def test_2d_list(self):
        ...
        n2 = node(2000)
        n0 = node(100, [n1, n2])
        r = [x.val for x in xiter(n0, gnxt={node : ChildAttr('sub')})]
    ...
    def test_hybrid(self):
        ...
        a = [n0, n1]
        r = [(idx.idx(), x) for x,idx in xiter(a, Index(), gnxt={node:ChildAttr('sub')} ) if isinstance(x, int)]
    ...
    def test_select_object(self):
        ...
        n1.sub = [1000,2000,3000]
        a = [n0, n1]
        r = [x.val for x in xiter(a, gnxt={node: ChildAttr('sub')}, sSelect='node' )]
        assert r == [100,200]
